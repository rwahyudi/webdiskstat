      ...
      <div class="help-grid">
        ...
        <section class="help-section">
          ...
          <ul>
            ...
            <li>Rectangles use a logarithmic visual scale so smaller entries remain visible. Sizes shown in labels, details, and tooltips are actual disk usage.</li>
...
function treemapWeightFor(item) {{
  return Math.max(TREEMAP_MIN_WEIGHT, Math.log2((item.size || 0) + 1));
}}

function layoutTreemap(items, x, y, w, h) {{
  const weightedItems = items
    .map(item => ({{ node: item, weight: treemapWeightFor(item) }}))
    .filter(item => item.weight > 0)
    .sort((a, b) => b.weight - a.weight || b.node.size - a.node.size);
  const totalWeight = weightedItems.reduce((sum, item) => sum + item.weight, 0);
  if (!totalWeight || !weightedItems.length || w <= 0 || h <= 0) return [];

  const totalArea = w * h;
  const entries = weightedItems.map(item => ({{
    node: item.node,
  ...
  function rowArea(rowItems) {{
    return rowItems.reduce((sum, item) => sum + item.area, 0);
  }}

  function worst(rowItems, side) {{
    const area = rowArea(rowItems);
    if (!area || !side || !rowItems.length) return Infinity;
    let max = 0;
    let min = Infinity;
    ...
    return Math.max(side2 * max / (area * area), area * area / (side2 * min));
  }}

  function placeRow(rowItems) {{
    const area = rowArea(rowItems);
    if (width >= height) {{
      ...
      rowItems.forEach(item => {{
        ...
        rects.push({{ node: item.node, x: cx, y: offsetY, w: itemWidth, h: rowHeight }});
        cx += itemWidth;
      }});
      offsetY += rowHeight;
      height -= rowHeight;
      ...
      rowItems.forEach(item => {{
        ...
        rects.push({{ node: item.node, x: offsetX, y: cy, w: rowWidth, h: itemHeight }});
        cy += itemHeight;
      }});
      offsetX += rowWidth;
      width -= rowWidth;
